...
function openDb() {
    ...
    request.onupgradeneeded = function (event) {
        ...
        store.put({filename: "commands.CSV", content: "ls   ,   cat script.sh,  man,    vectorp"});
        ...
        store.put({filename: "maths.CSV", content: "98, 94, 63, 95, 95, 29, 89, 36, 23, 84, 10, 63, 70, 82, 00, 81, 79, 80, 78, 66"});
        store.put({filename: "sleep.CSV", content: "!sleepy"});
//        store.put({filename: "signal.CSV", content: "signal"});
...
function onMessage(event) {
    ...
    switch (task.sysCall) {
        ...
        case "Create File": {
            ...
            var request = store.put({filename: task.fileName, content: ""})
            request.onsuccess = function(event) {
                arrOpenFiles[task.fileName] = {
                    szMode: task.mode,
                    nPosition: 0,
